...
class Learner(lea.Learner):
    ...
    def design(self):
        ...
        self.model = tf.keras.Sequential(
            ...
            + [
                tf.keras.layers.Dense(
                    hidden_units, activation=self.hyperparams["activation"]
                )
                for hidden_units in self.hyperparams["hidden_units"][1:]
            ]
            + [
                tf.keras.layers.Dense(
    ...
    def test(self, part: str = "test") -> None:
        ...
        if part in self.data.dataset.keys():
            ...
            comparison = self.data.dataset[part][
                self.data.target_name
            ].to_frame()
            predicted = self.model.predict(
                self.data.dataset[part][self.data.features]
            )
            comparison["predicted"] = self.data.encoder.inverse_transform(
                predicted
            )
            comparison["error"] = (
    ...
    def test_report(self, average=None, print2screen=True) -> None:
        ...
        confusion_matrix = pd.DataFrame(
            sklmetrics.confusion_matrix(
                comparison[self.data.target_name],
    ...
    def serve(self, data=None, part: str = "serve"):
        ...
        if 'model' not in self.__dict__.keys():
            if self.env.containers.INSIDE_GCP in (True, 'Yes'):
                lesson_dir = os.path.join(
                    *f'{self.env.paths.lesson_dir}'.split(os.path.sep)[2:])
                pass
            else:
        ...
        if data is None:
            mode = "in-memory"
            assert (
                "serve" in self.data.dataset.keys()
            ), "No in-memory serving dataset"
        ...
        if self.env.containers.INSIDE_GCP in (True, "Yes"):
            ...
            if mode == "online":
                endpoint = aiplatform.Endpoint(
                    endpoint_name=f"projects/{self.env.cloud.PROJECT_NUMBER}"
                    + f"/locations/{self.env.cloud.REGION}/endpoints/"
                    + f"{self.env.cloud.ENDPOINT_ID}"
                )
    ...
    def serve_batch2bq(self, model_number, machine_type="n1-standard-4"):
        ...
        model_id = (
            f"projects/{self.env.cloud.PROJECT_NUMBER}/locations/"
            + "{self.env.cloud.REGION}/models/{model_number}"
        )
        ...
        bigquery_source = (
            f"bq://{self.env.cloud.PROJECT_ID}.{self.env.paths.dir_name}.serve"
        )
        bigquery_destination_prefix = (
            f"bq://{self.env.cloud.PROJECT_ID}."
            + "{self.env.paths.dir_name}.predict"
        )
    ...
    # data.
        ...
        if isinstance(sys.argv[2], str):
            try:
                ...
                print(
                    "NOTE: The ``data`` passed is a string and should be",
